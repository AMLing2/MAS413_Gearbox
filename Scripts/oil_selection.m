...
if exist(fullfile(export_import,'gear_sizes.mat'), 'file')
    ...
    "b_s1", "b_s2", "d_g3", "d_g4", "d_g1", "d_g2", "v_p_g1","v_p_g2", "v_p_g3", "v_p_g4",...
    "n_1", "n_2", "n_3","n_4")
end
...
%% Gear oil
%Gear one
Belastning_hastighetsfaktorer_g1 = ks(F_t1, b_s1, d_g1,i_s1) / v_p_g1;%[ (N*s) /  (mm^2*m) ]

%Gear two
Belastning_hastighetsfaktorer_g2 = ks(F_t2, b_s1, d_g2,i_s1) / v_p_g2;%[ (N*s) /  (mm^2*m) ]

%Gear three
Belastning_hastighetsfaktorer_g3 = ks(F_t3, b_s2, d_g3,i_s2) / v_p_g3;%[ (N*s) /  (mm^2*m) ]

%Gear four
Belastning_hastighetsfaktorer_g4 = ks(F_t4, b_s2, d_g4,i_s2) / v_p_g4;%[ (N*s) /  (mm^2*m) ]



format compact
fprintf('Load and speed factor for gear 1: %.1f [N·s]/[mm²·m]\n', Belastning_hastighetsfaktorer_g1);
fprintf('Load and speed factor for gear 2: %.1f [N·s]/[mm²·m]\n', Belastning_hastighetsfaktorer_g2);
fprintf('Load and speed factor for gear 3: %.1f [N·s]/[mm²·m]\n', Belastning_hastighetsfaktorer_g3);
fprintf('Load and speed factor for gear 4: %.1f [N·s]/[mm²·m]\n', Belastning_hastighetsfaktorer_g4);
fprintf('\n');
